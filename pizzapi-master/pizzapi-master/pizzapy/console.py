...
class ConsoleInput:
    ...
    @staticmethod
    def get_card_file(path= str(Path(__file__).resolve().parents[1]) + "/card"):
        f = []
        for (dirpath, dirnames, filenames) in walk(path):
            for file in filenames:
                print(file)
    ...
    def get_card():
        card_file = ConsoleInput.get_card_file()
        if len(card_file) == 0:
            print("No customer cards exist, please add new payment.\n")
            
        else:
            returning = input("Would you like to Mark to pay? [y,n]: ")

            if returning.strip().lower() in ["y", "yes"]:
                card = []
                for i in range(len(card_file)):
                    card = ConsoleInput.load_card(card_file[i])
                    #card.append(cards)
                    print(str(i+1) + ".")
                    #print(cards.card_expiry)
                    print(card)
    ...
    def load_card(filename):
        return CreditCard.load(filename)
    ...
    def get_customer():
        ...
        if len(customer_files) == 0:
            ...
            returning = input("Would you like to load an exisiting customer profile? [y,n]: ")
